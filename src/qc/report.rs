...
pub fn generate_qc_report(input_dir: &str, output_path: &str) -> Result<()> {
    ...
    // Load H5 file
    let h5_path = Path::new(input_dir).join("methrix_data.h5");
    if !h5_path.exists() {
        anyhow::bail!("H5 file not found: {}", h5_path.display());
    }

    let file = hdf5::File::open(&h5_path)
        .context("Failed to open H5 file")?;

    // Read coverage matrix from assays group
    let assays_group = file.group("assays")
        .context("Failed to open assays group")?;

    let cov_dataset = assays_group.dataset("cov")
        .context("Failed to open cov dataset")?;

    let cov_matrix: Vec<u16> = cov_dataset.read_raw()
        .context("Failed to read coverage data")?;

    // Get dimensions from the dataset
    let cov_space = cov_dataset.space()
        .context("Failed to get dataspace")?;
    ...
    let matrix_2d: Vec<Vec<u16>> = (0..n_samples)
        .map(|i| {
            (0..n_cpgs)
                .map(|j| cov_matrix[j + i * n_cpgs])
                .collect()
        })
        .collect();

    // Get sample names from colData
    let coldata_group = file.group("colData")
        .context("Failed to open colData group")?;

    let sample_dataset = coldata_group.dataset("sample_id")
        .context("Failed to open sample_id dataset")?;
    let sample_ids: Vec<hdf5::types::VarLenAscii> = sample_dataset.read_raw()
        .context("Failed to read sample IDs")?;

    let sample_names: Vec<String> =
        sample_ids.iter().map(|s: &hdf5::types::VarLenAscii| s.to_string()).collect();

    // Calculate stats
    let stats = crate::processing::stats::calculate_coverage_stats_from_vec(
        &matrix_2d,
        &sample_names,
    );
